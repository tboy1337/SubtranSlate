...
class Translator(ABC):
    ...
    @abstractmethod
    def translate_lines(self, text_list: List[str], src_lang: str, target_lang: str, 
...
class GoogleTranslator(Translator):
            ...
            try:
                ...
                if e.code == 429 or (e.code >= 500 and e.code < 600):
                    ...
                    if retry_count <= self.max_retries:
                        ...
                        raise RateLimitError(f"Translation service rate limited (HTTP {e.code}). Try again later.")
                else:
                ...
                if retry_count <= self.max_retries:
                    ...
                    raise TranslationError(f"Failed to connect to translation service after {self.max_retries} attempts: {e}")
            ...
            try:
                ...
                if response.status_code == 429:  # Rate limit error
                    ...
                    if retry_count <= self.max_retries:
                        ...
                        logger.error(f"Max retries reached after API server error: {response.status_code}")
                ...
                if retry_count <= self.max_retries:
                    ...
                    raise TranslationError(f"Failed to connect to Google Translation API after {self.max_retries} attempts: {e}")
    ...
    def __translate_without_api(self, text: str, src_lang: str, target_lang: str) -> str:
        ...
        for domain in domains:
            url = f"http://{domain}/translate_a/single?client=t" \
        ...
        try:
                ...
                if total_length > self.max_limited:
                    ...
                    if batch.strip():  # Only translate non-empty batches
                        ...
                        try:
                            ...
                            logger.warning("Rate limit detected during batch translation. Backing off for 30 seconds.")
                    ...
                    logger.info(f"Pausing for {delay:.2f}s between translation batches to avoid rate limiting")
                    time.sleep(delay)
            ...
            if last_batch.strip():  # Only translate non-empty batches
                ...
                try:
                    ...
                    logger.warning("Rate limit detected during final batch translation. Backing off for 30 seconds.")
                    time.sleep(30)
